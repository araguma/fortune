...
const command = new Command().setName('view').setDescription('Inspect a stock').setGroup(Group.Trade)
...
command
    ...
    .addUserOption((option) => option.setName('as').setDescription('View as user').setRequired(false))

command.setChatInputCommandHandler(async (interaction) => {
    const symbol = interaction.options.getString('symbol', true)
    const timeframe = (interaction.options.getString('timeframe') ?? '1D') as keyof typeof timeframes
...
command.setButtonHandler(async (interaction) => {
    ...
    switch (tag.getAction(true)) {
        case 'update': {
            ...
            const timeframe = tag.getData('timeframe', true) as keyof typeof timeframes
...
command.setStringSelectMenuHandler(async (interaction) => {
    ...
    switch (tag.getAction(true)) {
        case 'timeframe': {
            const symbol = tag.getData('symbol', true)
            const timeframe = (interaction.values[0] ?? '1D') as keyof typeof timeframes
            const userId = tag.getData('userId', true)
