...
const command = new Command().setName('prediction').setDescription('Open a new prediction').setGroup(Group.Prediction)

command
    .addStringOption((option) => option.setName('prompt').setDescription('Prediction prompt').setRequired(true))
    .addStringOption((option) => option.setName('options').setDescription('Options seperated by |').setRequired(true))
    .addNumberOption((option) => option.setName('minimum').setDescription('Minimum bet').setRequired(true))
    ...
    switch (tag.getAction(true)) {
        ...
        case 'predict': {
            ...
            const optionInput = new TextInputBuilder()
                .setCustomId('option')
                .setLabel(`Select option [1, ${prediction.model.options.length}]`)
                .setStyle(TextInputStyle.Short)
                .setRequired(true)
            const amountInput = new TextInputBuilder()
                .setCustomId('amount')
                .setLabel(`Input amount [${prediction.model.minimum}, ${format.number(client.model.balance)}]`)
                .setStyle(TextInputStyle.Short)
                .setRequired(true)
            const row1 = new ActionRowBuilder<TextInputBuilder>().addComponents(optionInput)
        ...
        case 'settle': {
            ...
            const optionInput = new TextInputBuilder()
                ...
                .setStyle(TextInputStyle.Short)
                .setRequired(true)
            const row = new ActionRowBuilder<TextInputBuilder>().addComponents(optionInput)
...
command.setModalSubmitHandler(async (interaction) => {
    ...
    const prediction = await Prediction.findById(tag.getData('predictionId', true))

    switch (tag.getAction(true)) {
        case 'predict': {
            const userId = interaction.user.id
            const client = await Client.getClientByUserId(userId)
            const option = parseInt(interaction.fields.getTextInputValue('option')) - 1
        ...
        case 'settle': {
            ...
            const clients = Object.fromEntries(
                await Promise.all(
                    Array.from(prediction.model.bets.keys()).map(async (userId) => {
                        return [userId, (await Client.getClientByUserId(userId)).model] as const
                    }),
                ),
            )
            const option = parseInt(interaction.fields.getTextInputValue('option')) - 1
